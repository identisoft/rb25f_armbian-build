...
fetch_from_repo()
{
	...
	if [[ $Build_Type == "release" && $make_tag == "yes" ]]; then
		...
		if [[ $tag_exist == "refs/${ref_type}s/${ref_name}" ]]; then
			...
			display_alert "Release tag not found, switching to master to create tag" "" ""
	...
	if [[ -f .gitmodules ]]; then
		...
		for i in $(git config -f .gitmodules --get-regexp path | awk '{ print $2 }'); do
			...
			fetch_from_repo "$surl" "$workdir/$i" "$target_dir" "$sref" "$make_tag"
		done
	fi

	if [[ $create_tag == "yes" ]]; then
		if [[ $make_tag == "yes" ]]; then
			display_alert "Creating tag and switching to new tag" "" ""
			git tag ${product_issue}
			git push origin --tags
			ref_type=$old_ref_type
			ref_name=$old_ref_name
			case $ref_type in
				branch)
				# TODO: grep refs/heads/$name
					local remote_hash=$(git ls-remote -h $url "$ref_name" | head -1 | cut -f1)
			...
			display_alert "Tag creation is disabled for the repository" "" "wrn"
		fi
	fi
} #############################################################################
...
#--------------------------------------------------------------------------------------------------------------------------------
	...
	cat <<-EOF > $1
	--------------------------------------------------------------------------------
	Title:			RB25F $REVISION ${BOARD^} $DISTRIBUTION $RELEASE $BRANCH
	Kernel:			Linux $VER
...
prepare_host()
{
	...
	bison libbison-dev flex libfl-dev smbclient cmake make"
  ...
  if [[ $SYNC_CLOCK != no ]]; then
	...
	if [[ $CHECK_FREE_SPACE != no ]]; then
		...
		display_alert "Free space checking is disabled" "" "wrn"
	fi
	}
...
{ 
  local url="https://$GitHostName/$1"
	...
	#mkdir -p $SRC/userpatches/overlay/sources/$workdir
	#cd $SRC/userpatches/overlay/sources/$workdir
	mkdir -p ${SRC}/${target_dir}/${workdir}
	cd ${SRC}/${target_dir}/${workdir}
	# check if existing remote URL for the repo or branch does not match current one
	# may not be supported by older git versions
	local current_url=$(git remote get-url origin 2>/dev/null)
	if [[ -n $current_url && $(git rev-parse --is-inside-work-tree 2>/dev/null) == true && \
