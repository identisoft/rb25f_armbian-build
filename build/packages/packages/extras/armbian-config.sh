...
compile_armbian-config()
{
	...
	if [[ $Build_Type == "release" ]]; then
		fetch_from_repo "http://github.com/identisoft/rb25f_armbian-config" "armbian-config" "tag:$product_issue" "cache/sources" "no" "yes"
	else
		fetch_from_repo "http://github.com/identisoft/rb25f_armbian-config" "armbian-config" "branch:master" "cache/sources" "no" "no"
	fi
...
chroot $SDCARD /bin/bash -c "apt install -q -y iperf3 debconf-utils html2text dirmngr"
